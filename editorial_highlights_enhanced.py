#!/usr/bin/env python3
"""
Enhanced Editorial Highlights Generator with Real Video Screenshots
Creates video highlights with actual screenshots from the video at each segment
"""
...
from core.enhanced_processor import EnhancedVideoProcessor
from core.summarizer import SummaryGenerator  
from core.github_deploy import GitHubDeployer
from core.config_manager import ConfigManager


class EnhancedEditorialWorkflow:
    def __init__(self):
        self.processor = EnhancedVideoProcessor()  # Use enhanced processor
    ...
    def run(self):
        """Main interactive workflow with screenshot extraction"""
        print("\n🎬 Enhanced Editorial Video Highlights Generator")
        print("   Now with real video screenshots!")
        print("=" * 50)
        
        try:
            # Get inputs with minimal prompts
            video_url = self.get_url()
            self.current_video_url = video_url  # Store for screenshot extraction
            ...
            # Generate output with screenshots
            print("\n✓ Building page with video screenshots...")
    ...
    def generate_highlights(self, segments: List[Dict], video_url: str) -> str:
        """Generate the highlights HTML page with real screenshots"""
        ...
        # Extract real screenshots or generate thumbnails
        print("✓ Extracting video screenshots...")
        screenshot_results = self.processor.extract_all_screenshots(
            video_url, segments, output_path
        )
        
        # Generate HTML
        html_content = self.generate_html(segments, video_id, video_url, screenshot_results)
    ...
    def generate_html(self, segments: List[Dict], video_id: str, video_url: str, 
                     screenshot_results: Dict[int, bool]) -> str:
        """Generate clean editorial HTML with screenshot indicators"""
        cards_html = ""
        for i, segment in enumerate(segments, 1):
            ...
            # Add indicator if real screenshot was used
            screenshot_indicator = ""
            if screenshot_results.get(i, False):
                screenshot_indicator = '<div class="screenshot-badge">📸 Real Screenshot</div>'
            
            cards_html += f"""
            <div class="card {emphasis_class}" data-seconds="{seconds}">
                <div class="image-container">
                    <img src="thumb_{i:02d}.jpg" alt="Segment {i}" loading="lazy">
                    {screenshot_indicator}
                </div>
...
<body>
    ...
    <script>
        ...
        function setupCardClickHandlers() {{
            ...
            document.querySelectorAll('.card').forEach(card => {{
                card.addEventListener('click', function(e) {{
                    ...
                    // Get seconds from data attribute
        ...
        // Also set up handlers when DOM is ready (in case player loads slowly)
        document.addEventListener('DOMContentLoaded', function() {{
            setTimeout(setupCardClickHandlers, 1000);
        }});
    </script>
</body>
</html>"""
    ...
    def deploy_to_github(self, output_dir: str):
        ...
        # Get commit message
        message = input("Message: ").strip() or "Add highlights with screenshots"
...
def main():
    """Entry point"""
    workflow = EnhancedEditorialWorkflow()
    workflow.run()
